...
import { tamilNaduDistricts } from '../data/districtData';
...
const SimpleFilterPanel: React.FC<SimpleFilterPanelProps> = ({
  ...
  const getActiveFilterCount = () => {
    ...
    if (activeFilters.district) count++;
      ...
      <CardContent className="space-y-4">
        ...
        {getActiveFilterCount() > 0 && (
          <div className="flex flex-wrap gap-2 p-3 bg-primary/5 rounded-lg">
            ...
            {activeFilters.district && <Badge>{tamilNaduDistricts.find(d => d.id === activeFilters.district)?.name}<X onClick={() => handleFilterChange('district', undefined)} /></Badge>}
        ...
        {isExpanded && (
          <>
            ...
            <div className="space-y-2">
              <Label><MapPin className="w-4 h-4 text-primary" /> District</Label>
              <Select value={activeFilters.district || 'all'} onValueChange={(v) => handleFilterChange('district', v === 'all' ? undefined : v)}>
                <SelectTrigger><SelectValue placeholder="Select district..." /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">All Districts</SelectItem>
                  {tamilNaduDistricts.map(d => <SelectItem key={d.id} value={d.id}>{d.name}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-4">
              <Label><BarChart3 className="w-4 h-4 text-primary" /> NPK Ranges</Label>
              <div className="space-y-3">
                <div>
                  <Label className="text-sm">Nitrogen (N): {npkRanges.nitrogen[0]} - {npkRanges.nitrogen[1]}</Label>
                  <Slider
                    value={npkRanges.nitrogen}
                    onValueChange={(values) => handleNpkChange('nitrogen', values)}
                    max={100}
                    min={0}
                    step={1}
                    className="mt-2"
                ...
                <div>
                  <Label className="text-sm">Phosphorus (P): {npkRanges.phosphorus[0]} - {npkRanges.phosphorus[1]}</Label>
                  <Slider
                    value={npkRanges.phosphorus}
                    onValueChange={(values) => handleNpkChange('phosphorus', values)}
                    max={100}
                    min={0}
                    step={1}
                    className="mt-2"
                ...
                <div>
                  <Label className="text-sm">Potassium (K): {npkRanges.potassium[0]} - {npkRanges.potassium[1]}</Label>
                  <Slider
                    value={npkRanges.potassium}
                    onValueChange={(values) => handleNpkChange('potassium', values)}
                    max={100}
                    min={0}
                    step={1}
                    className="mt-2"
            ...
            <div className="space-y-2">
              <Label><Activity className="w-4 h-4 text-primary" /> NDVI Range: {ndviRange[0]} - {ndviRange[1]}</Label>
              <Slider
                value={ndviRange}
                onValueChange={handleNdviChange}
                max={1}
                min={0}
                step={0.01}
                className="mt-2"
