...
import { FilterCriteria } from './SimpleFilterPanelWithDistrict';
import { tamilNaduDistricts, filterLocationsByDistrict } from '../data/districtData';
...
const InteractiveMap: React.FC<InteractiveMapProps> = ({
  ...
  const filteredLocations = useMemo(() => {
    ...
    // First filter by district if selected
    let districtFilteredLocations = mockLocations;
    if (filters.district) {
      districtFilteredLocations = filterLocationsByDistrict(mockLocations, filters.district);
    }

    // Then apply other filters
    return filterLocations(districtFilteredLocations, filters);
  }, [filters]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      tamilNaduDistricts.forEach(district => {
        const districtCoords = district.coordinates.map(coord => [coord[1], coord[0]]); // Convert to [lat, lng]

        const districtPolygon = L.polygon(districtCoords, {
          color: '#666',
          weight: 1,
          opacity: 0.5,
          fillColor: '#999',
        ...
        // Add popup for district
        districtPolygon.bindPopup(`<strong>${district.name}</strong><br/>Area: ${district.area} sq km`);

        districtLayersRef.current.push(districtPolygon);
      });
  ...
  // Update district highlighting when district filter changes
  useEffect(() => {
    ...
    if (filters.district) {
      const selectedDistrict = tamilNaduDistricts.find(d => d.id === filters.district);
      if (selectedDistrict) {
        const districtLayer = districtLayersRef.current.find(layer => {
          return layer.getBounds().contains([selectedDistrict.center[0], selectedDistrict.center[1]]);
        });
        ...
        if (districtLayer) {
          ...
          leafletMapRef.current.setView(selectedDistrict.center, 9);
        }
      }
    }
  }, [filters.district, isMapReady]);
          ...
          <p className="text-xs text-muted-foreground">
            {filters.district && (
              <span className="block mb-1">
                <strong>District:</strong> {tamilNaduDistricts.find(d => d.id === filters.district)?.name}
              </span>
            )}
